...
async function main() {
    ...
    const appointments = await Promise.all(appointmentsData.map(data => prisma.appointment.create({ data })));
    ...
    const orders = await Promise.all(ordersData.map(data => prisma.order.create({
        data: {
            ...data,
            items: {
                create: [
